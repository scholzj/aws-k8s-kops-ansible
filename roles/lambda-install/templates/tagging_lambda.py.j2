...
# Configuration
region = "eu-west-1"
cluster = "dave-dev.dbg-devops.com"
...
def prepare_new_tags(tags):
    ...
    if find_tag(tags, "Application") != tag_Application:
        newTags.append({'Key': 'Application', 'Value': tag_Application})

    if find_tag(tags, "Confidentiality") != tag_Confidentiality:
        newTags.append({'Key': 'Confidentiality', 'Value': tag_Confidentiality})

    if find_tag(tags, "CostCenter") != tag_CostCenter:
        newTags.append({'Key': 'CostCenter', 'Value': tag_CostCenter})

    if find_tag(tags, "Owner") != tag_Owner:
        newTags.append({'Key': 'Owner', 'Value': tag_Owner})
